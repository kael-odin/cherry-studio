 ...
 * values no filesystem can hold — `a\0b` and a 300-character segment are both
 * unanchored and non-empty — because segment legality only exists per platform.
 *
 * The two branches genuinely differ in both directions: `a<b` is POSIX-only, and
 * a 150-char name, a `\`, then another 150-char name is Windows-only (two legal
 * segments there, one over-long segment on POSIX).
 *
