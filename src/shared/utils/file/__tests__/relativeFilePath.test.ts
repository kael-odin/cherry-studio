...
describe('PosixRelativeFilePathSchema — structure', () => {
  ...
  it('rejects what no filesystem could hold, which a structure-only brand would admit', () => {
    // The reason there is no standalone "structurally relative" brand: both of these
    // are non-empty and unanchored, and neither can exist on disk.
    expect(accepts(PosixRelativeFilePathSchema, 'a\0b')).toBe(false)
    expect(accepts(PosixRelativeFilePathSchema, 'x'.repeat(300))).toBe(false)
  })
...
describe('PosixRelativeFilePathSchema', () => {
  it.each([
